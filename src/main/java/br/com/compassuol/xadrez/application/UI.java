...
public class UI {
    ...
    public static PosicaoXadrez lerPosicaoXadrez(Scanner sc){
        ...
        try{
            ...
            throw new InputMismatchException("Error ao ler posição Xadrez. Valores válidos são de a1 to h8");
        }
    }

    private static void printCapturedPieces(List<PecaXadrez> captured){
        List<PecaXadrez> white = captured.stream().filter(x -> x.getColor() == Color.WHITE).collect(Collectors.toList());
        List<PecaXadrez> black = captured.stream().filter(x -> x.getColor() == Color.BLACK).collect(Collectors.toList());
